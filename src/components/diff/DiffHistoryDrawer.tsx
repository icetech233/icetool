/**
 * DiffHistoryDrawer —— 文本差异历史记录抽屉（右侧滑入，移动端底部滑起）。
 *
...
import { HISTORY_MAX, type DiffHistoryItem } from '../../utils/storage/diffDB';

interface DiffHistoryDrawerProps {
  open: boolean;
  items: DiffHistoryItem[];
  currentLeftText: string;
...
export default function DiffHistoryDrawer({
  ...
  const startEdit = (item: DiffHistoryItem) => {
  ...
  const askLoad = (item: DiffHistoryItem) => {
    const hasContent = currentLeftText.trim().length > 0 || currentRightText.trim().length > 0;
    const isSame = currentLeftText === item.leftText && currentRightText === item.rightText;
    if (!hasContent || isSame) {
      onLoad(item);
      onClose();
      return;
    }
          ...
          >
            <header className="flex items-center justify-between border-b border-border px-4 py-3">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-semibold text-foreground">文本差异历史记录</h3>
